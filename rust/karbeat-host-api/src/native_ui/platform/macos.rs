...
pub struct MacOsNativeUi;
pub struct MacOsNativeWindow {
    id: NativeWindowId,
}

fn unavailable<T>() -> Result<T, NativeUiError> {
    Err(NativeUiError::PlatformBackendUnavailable("macOS"))
}

impl NativeWindow for MacOsNativeWindow {
...
impl NativeUiPlatform for MacOsNativeUi {
    type Window = MacOsNativeWindow;
