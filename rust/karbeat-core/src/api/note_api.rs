...
pub fn add_note(
    ...
    // 1. Mutate state
    let note = ctx.app_state.add_note_to_pattern(pattern_id, key, start_tick, duration)?;
...
pub fn delete_note(ctx: &mut DawContext, pattern_id: PatternId, note_id: NoteId) -> anyhow::Result<Note> {
    // 1. Mutate state
    let note = ctx.app_state.delete_note_from_pattern(pattern_id, note_id)?;
...
pub fn move_note(
    ...
    // 1. Mutate state
    let (note, old_tick, old_key) = ctx.app_state.move_note_in_pattern(pattern_id, note_id, new_start_tick, new_key)?;
...
pub fn resize_note(
    ...
    // 1. Mutate state
    let (note, old_duration) = ctx.app_state.resize_note_in_pattern(pattern_id, note_id, new_duration)?;
...
pub fn add_notes_batch(
    ...
    // 1. Mutate state
    let added_notes = ctx.app_state.add_notes_to_pattern_batch(pattern_id, &notes_data)?;
...
pub fn delete_notes_batch(ctx: &mut DawContext, pattern_id: PatternId, note_ids: Vec<NoteId>) -> anyhow::Result<()> {
    // 1. Mutate state
    let deleted_notes = ctx.app_state.delete_notes_from_pattern_batch(pattern_id, &note_ids)?;
...
pub fn move_notes_batch(
    ...
    // 1. Mutate state
    let moved_data = ctx.app_state.move_notes_in_pattern_batch(pattern_id, &updates)?;
...
pub fn resize_notes_batch(
    ...
    // 1. Mutate state
    let resized_data = ctx.app_state.resize_notes_in_pattern_batch(pattern_id, &updates)?;
