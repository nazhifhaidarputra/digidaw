//! Platform VST3 loop services. Steinberg `IRunLoop` is exposed only on Linux.
...
impl RunLoop {
    ...
    fn pump_linux(&self) {
        ...
        for handler in due {
            ...
            {
                // SAFETY: retained handler is dispatched on its registering UI thread.
        ...
        for event in events {
            ...
            // SAFETY: one writable pollfd and a zero timeout cannot block the UI loop.
            let ready = unsafe { poll(&raw mut fd, 1, 0) };
            if ready > 0
                && fd.revents & 1 != 0
                && self.events.borrow().iter().any(|registered| {
                    registered.fd == event.fd
                        && registered.handler.as_ptr() == event.handler.as_ptr()
                })
            {
...
impl IRunLoopTrait for RunLoop {
    unsafe fn registerEventHandler(
        ...
        // SAFETY: plugin supplies a live handler retained until unregister.
        ...
        if events
            .iter()
            .any(|event| event.fd == fd && event.handler.as_ptr() == handler.as_ptr())
        {
            return kResultFalse;
        }
        events.push(Event {
    ...
    unsafe fn unregisterEventHandler(&self, handler: *mut IEventHandler) -> tresult {
        ...
        events.retain(|event| event.handler.as_ptr() != handler);
    ...
    unsafe fn unregisterTimer(&self, handler: *mut ITimerHandler) -> tresult {
        ...
        timers.retain(|timer| timer.handler.as_ptr() != handler);
    ...
    ($target:ty) => {
        impl vst3::Steinberg::Linux::IRunLoopTrait for $target {
            unsafe fn registerEventHandler(
                ...
                // SAFETY: forwards unchanged VST3 ABI arguments to the shared UI loop.
            ...
            unsafe fn unregisterEventHandler(
                ...
                // SAFETY: forwards the plugin-owned handler identity.
            ...
            unsafe fn registerTimer(
                ...
                // SAFETY: forwards the unchanged handler and timer interval.
