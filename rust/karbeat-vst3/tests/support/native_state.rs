...
fn run<T>(future: impl Future<Output = Result<T, HostError>>) -> Result<T, HostError> {
...
pub fn exercise_state_gateway(descriptor: PluginDescriptor, config: ProcessingConfig) {
    ...
    let worker = std::thread::spawn(move || {
        ...
        assert!(matches!(
            run(native::call(move |owner| owner.create_prepared(
                &failed_descriptor,
                &failed_config,
                Some(&corrupt)
            ))),
        ...
        run(native::call(move |owner| owner.host.resume(id))).unwrap();
        ...
        for _ in 0..3 {
            ...
            let state = loop {
                match run(native::capture_state(id)) {
            ...
            assert!(run(native::call(move |owner| owner.host.is_processing(id))).unwrap());
            run(native::restore_state(id, state)).unwrap();
            assert!(run(native::call(move |owner| owner.host.is_processing(id))).unwrap());
        }
        let duplicate = run(native::duplicate_instance(id)).unwrap();
        let duplicate_id = duplicate.instance;
        assert_ne!(duplicate_id, id);
        assert!(!duplicate.state.component.is_empty());
        run(native::call(move |owner| {
        ...
        let sibling = run(native::duplicate_instance(id)).unwrap();
        ...
        let offline = run(native::prepare_offline_instance(id, 44_100, 256, 2)).unwrap();
        ...
        let deadline = Instant::now() + Duration::from_secs(5);
        while run(native::call(move |owner| {
            owner.host.is_processing(offline_id)
        }))
        .is_ok()
        {
            assert!(
        ...
        let deadline = Instant::now() + Duration::from_secs(5);
        while run(native::call(move |owner| owner.host.is_processing(id))).is_ok() {
            assert!(
                Instant::now() < deadline,
        ...
        let abandoned = run(native::call(move |owner| {
            owner.create_prepared(&descriptor, &config, None)
        }))
        .unwrap();
        ...
        let deadline = Instant::now() + Duration::from_secs(5);
        loop {
            match run(native::capture_state(abandoned_id)) {
