...
pub struct EnvelopeSettings {
    #[param(id = "attack", name = "Attack", group = "Envelope", min = 1.0, max = 10000.0, default = 10.0, step = 1.0)]
    pub attack_ms: f32,

    #[param(id = "decay", name = "Decay", group = "Envelope", min = 1.0, max = 10000.0, default = 200.0, step = 1.0)]
    pub decay_ms: f32,

    #[param(id = "sustain", name = "Sustain", group = "Envelope", min = 0.0, max = 1.0, default = 0.7, step = 0.01)]
    pub sustain_level: f32,

    #[param(id = "release", name = "Release", group = "Envelope", min = 1.0, max = 10000.0, default = 500.0, step = 1.0)]
    pub release_ms: f32,
}
...
impl Envelope for EnvelopeSettings {
    fn attack_ms(&self) -> f32 { self.attack_ms.get() }
    fn decay_ms(&self) -> f32 { self.decay_ms.get() }
    fn sustain_level(&self) -> f32 { self.sustain_level.get() }
    fn release_ms(&self) -> f32 { self.release_ms.get() }
}
...
pub struct AdvancedEnvelopeSettings {
    #[param(id = "delay", name = "Delay", group = "Envelope", min = 0.0, max = 5000.0, default = 0.0, step = 1.0)]
    pub delay_ms: f32,
    
    #[param(id = "attack", name = "Attack", group = "Envelope", min = 1.0, max = 10000.0, default = 10.0, step = 1.0)]
    pub attack_ms: f32,
    
    #[param(id = "hold", name = "Hold", group = "Envelope", min = 0.0, max = 5000.0, default = 0.0, step = 1.0)]
    pub hold_ms: f32,
...
impl Envelope for AdvancedEnvelopeSettings {
    ...
    fn sustain_level(&self) -> f32 { self.sustain_level.get() }
    fn release_ms(&self) -> f32 { self.release_ms.get() }
    fn peak_level(&self) -> f32 { self.peak_level.get() }
}
